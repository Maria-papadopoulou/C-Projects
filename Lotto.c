...
#include <stdlib.h>
#define lim 6
int numberOfN(int, int *);
int input();
int *matrix(int);
void numbersX1X2(int , int);
void numbersY1Y2(int, int);
int search(int *, int);
void sort(int *, int );
...
int input()
{
	...
	do 
	{
		printf ("Δώσε το πλήθος των αριθμών[7...49]:");
		scanf ("%d", &n);
		printf("\n");
	}
	while (n < 7 || n > 49);
...
for (i =0; i <c; i++)
	{
		do
		{
			...
			printf("%2doς αριθμός στο [1...49]:",i+1);
		    scanf("%d",&*(matA + i));
		    src = search (matA, i);
		}
...
void numbersX1X2(int x1, int x2)
{
	int i, j;
	printf("\n");
	printf("Εισάγετε ένα ζεύγος αριθμών Χ1, Χ2.\n");
	do
		{
		printf("Εισαγωγή Χ1 (0<=X1<=X2<=6):");
		scanf("%d", &x1);
		}
		while(x1<0 || x1>6);
do 
{
	printf("Εισαγωγή Χ2 (0<=X1<=X2<=6):");
		scanf("%d", &x2);
}
while (x2<x1 || x1>6);	
}
void numbersY1Y2(int y1, int y2)
{
printf("\n");
	printf("Εισάγετε ένα ζεύγος αριθμών Y1, Y2.\n");
	do
	{
		printf("Εισαγωγή Y1 τέτοιο ώστε (21<=Υ1<=Υ2<=279):");
		scanf("%d", &y1);
		
	} while(y1<21 || y1>279);
	do{

		printf("Εισαγωγή Y2 τέτοιο ώστε (21<=Υ1<=Υ2<=279):");
		scanf("%d", &y2);
